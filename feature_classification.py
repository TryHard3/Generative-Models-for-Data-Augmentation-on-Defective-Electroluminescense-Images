...
if __name__ == '__main__':
    ###### SCENARIO 1 ######
    print('###### SCENARIO 1 ######')
    x_train, y_train, x_test, y_test, x_val, y_val = utils.make_dataset(scenario='1')

    s1 = TrainClassifiers(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, x_val=x_val, y_val=y_val)
    s1.train_all()

    ###### SCENARIO 2 ######
    print('###### SCENARIO 2 ######')
    x_train, y_train, x_test, y_test, x_val, y_val = utils.make_dataset(scenario='2')

    s2 = TrainClassifiers(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, x_val=x_val, y_val=y_val)
    s2.train_all()

    ###### SCENARIO 3 ######
    print('###### SCENARIO 3 ######')
    x_train, y_train, x_test, y_test, x_val, y_val = utils.make_dataset(scenario='3')

    s3 = TrainClassifiers(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, x_val=x_val, y_val=y_val)
    s3.train_all()

    ###### VAE + SCENARIO 3 ######
    print('###### VAE + SCENARIO 3 ######')
    vae_samples = pd.read_pickle('data/vae_samples_good.pkl')  # Output of CVAE
    # 31,000 of each failers goes to train
    vae_y_train = vae_samples['labels'].iloc[0:62000].astype(str).astype(int).values
    vae_x_train = vae_samples.drop(['labels'], axis=1).iloc[0:62000]
    # 9000 of each failers goes to validation
    vae_y_val = vae_samples['labels'].iloc[62000:79999].astype(str).astype(int).values
    vae_x_val = vae_samples.drop(['labels'], axis=1).iloc[62000:79999]
    # Get the original dataset and combine with the CVAE
    x_train, y_train, x_test, y_test, x_val, y_val = utils.make_dataset(scenario='4')
    x_train = x_train.append(vae_x_train)
    y_train = np.concatenate((y_train, vae_y_train))
    x_val = x_val.append(vae_x_val)
    y_val = np.concatenate((y_val, vae_y_val))

    s3_vae = TrainClassifiers(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, x_val=x_val, y_val=y_val)
    s3_vae.train_all()

    ###### GAN + SCENARIO 3 ######
    print('###### GAN + SCENARIO 3 ######')
    gan_train_samples = pd.read_pickle('data/Training_features.pkl')  # Output of GAN for training
    gan_train_samples = gan_train_samples[gan_train_samples.columns[256:270]]
    gan_train_samples = gan_train_samples.iloc[0:62000]
    gan_val_samples = pd.read_pickle('data/validation_features.pkl')  # Output of GAN for validation
    gan_val_samples = gan_val_samples[gan_val_samples.columns[256:270]]
    gan_val_samples = gan_val_samples.iloc[0:18000]
    gan_y_train = gan_train_samples['labels']
    gan_x_train = gan_train_samples.drop(['mu','labels'], axis=1)
    gan_y_val = gan_val_samples['labels']
    gan_x_val = gan_val_samples.drop(['mu', 'labels'], axis=1)
    x_train, y_train, x_test, y_test, x_val, y_val = utils.make_dataset(scenario='4')
    x_train = x_train.append(gan_x_train)
    y_train = np.concatenate((y_train, gan_y_train))
    x_val = x_val.append(gan_x_val)
    y_val = np.concatenate((y_val, gan_y_val))

    s3_gan = TrainClassifiers(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, x_val=x_val, y_val=y_val)
    s3_gan.train_all()
